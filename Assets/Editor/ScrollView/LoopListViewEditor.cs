...
namespace SuperScrollView
{
    ...
    public class LoopListViewEditor : UnityEditor.Editor
    {
        ...
        void ShowPreviewList(LoopListView listView)
        {
            ...
            if (GUILayout.Button(label))
            {
                if (preview == null)
                {
                    ...
                    //隐藏遮罩
                    UnityEngine.EventSystems.UIBehaviour mask = listView.ScrollRect.viewport.GetComponent<RectMask2D>();
                    if (mask == null)
                    {
                        mask = listView.ScrollRect.viewport.GetComponent<Mask>();
                    }
                    mask.enabled = false;
