...
public class SpriteSelectTool : EditorWindow
{
    #region 属性变量定义
    /// <summary>
    /// 选中回调
    ...
    public static SpriteSelectTool Open(Sprite sprite = null)
    {
        ...
        if (sprite != null)
        {
            string initDir = EditorUtils.GetAssetFloderName(sprite);
            if (initDir != "Resources")//可能是使用了原生贴图
            {
                rt.SelectDirName = EditorUtils.GetAssetFloderName(sprite);
            }
        }
        return rt;
    }
    ...
    {
        allAtlasDics = FileUtils.GetDirs(SpriteAtlasTool.AtlasSpriteFullPath);
    ...
    protected  void OnDrawContent()
    {
        ...
        if (allAtlasDicsPath != null)
        {
            ...
            {
                allSprites.Clear();
                // Prefer searching for sprites; fall back to textures if none found
                tempGuids = AssetDatabase.FindAssets("t:Sprite", new string[] { selectDirPath });
                if (tempGuids == null || tempGuids.Length == 0)
                {
                ...
                for (int i = 0; i < tempGuids.Length; i++)
                {
                    ...
                    // Try load as Sprite first, then try to extract sprites from texture
                    ...
                    // If not a Sprite asset, try to load Texture and get its sub-assets (multiple sprites)
    ...
    // Main OnGUI for EditorWindow to render menus and content
    private void OnGUI()
    {
        // Draw top controls
        OnDrawMenus();
    ...
    private void DrawImages()
    {
        ...
        for (int i = 0; i < allSprites.Count; i++)
        {
            ...
            if (EditorUtils.SpriteButton(rect, allSprites[i], style, GUILayout.Width(50), GUILayout.Height(50)))
            {
