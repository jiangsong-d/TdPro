...
{
    public delegate void DestroyGridListHandle(LoopGridView loopGridView);
    ...
    public class LoopGridView : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
    {
        ...
        public static DestroyGridListHandle onViewDestroyEvent = null;
        ...
        private Vector2 AnimOffset; //动画偏移
        private float AnimTime;   //动画时间
        private float AnimInterval; //动画间隔
        /// <summary>
        /// 简单的列表动画,拿所有节点做动画目前只有单个如果需要组动画直接去组个数分组就行了
        /// </summary>
        /// <param name="offset">动画偏移</param>
        /// <param name="time">动画时间</param>
        /// <param name="interval">下一个动画间隔</param>
        public void PlayInitAnim(Vector2 offset, float time, float interval)
        {
            if (PlayCoroutine != null) StopCoroutine(PlayCoroutine);
            foreach (var VARIABLE in TweenList)
            {
                VARIABLE.Kill();
            }
            TweenList.Clear();
            ...
            if (mItemTotalCount > 0)
            {
                ...
                {
                    var it = GetShownItemByItemIndex(i);
                    if (it)
                    {
                        it.CachedRectTransform.anchoredPosition3D = GetItemPos(it.Row, it.Column);
                        it.gameObject.AddComponent<CanvasGroup>().alpha = 0;
                    }
                }
                PlayCoroutine = StartCoroutine(OnPlay());
            }
        }
        ...
        private IEnumerator OnPlay()
        {
            for (int i = 0; i < mItemTotalCount; i++)
            {
                ...
                {
                    if (!this) yield break;
                    var pos = GetItemPos(it.Row, it.Column);
                    it.CachedRectTransform.anchoredPosition3D = pos + AnimOffset;
                    // TweenList.Add(it.CachedRectTransform.DOAnchorPos3D(pos, AnimTime));
                    //TweenList.Add(it.DOFade(0, 1, AnimTime));
        ...
        public int ItemViewFirstIndex
        {
            get
            {
                return mItemGroupList.Count > 0 ? mItemGroupList[0].First.ItemIndex : -1;
            }
        }
        ...
        public int ItemViewLastIndex
        {
            get
            {
                return mItemGroupList.Count > 0 ? mItemGroupList[mItemGroupList.Count - 1].Last.ItemIndex : -1;
            }
        }

        public int FixedRowOrColumnCount
        {
            get
            {
        ...
        public GridFixedType GridFixedType
        {
            get
            {
                return mGridFixedType;
            }
        }
        ...
        */
            ...
            LoopGridViewInitParam initParam = null)
        {
            ...
            {
                LogUtlis.Error("LoopGridView.InitListView method can be called only once.");
                return;
            }
            mListViewInited = true;
            if (itemTotalCount < 0)
            {
                LogUtlis.Error("itemTotalCount is  < 0");
                itemTotalCount = 0;
            }
        ...
        */
        public void SetGridItemCount(int itemCount, bool resetPos = true)
        {
            if (itemCount < 0)
            {
                return;
        ...
        */
        ...
        public void UpdateItemSize()
        {
            ...
            {
                LogUtlis.Error("Error, ItemSize is invaild.");
            }
        ...
        {
            if (onViewDestroyEvent != null)
            {
                onViewDestroyEvent(this);
            }
        }
    }
