...
namespace SuperScrollView
{
    ...
    public delegate void DestroyLoopListHandle(LoopListView loopListView);
    ...
    public class LoopListView : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
    {
        ...
        public System.Action<LoopListView, LoopListViewItem> mOnSnapItemFinished = null;
        public System.Action<LoopListView, LoopListViewItem> mOnSnapNearestChanged = null;
        ...
        public static DestroyLoopListHandle onLoopListViewDestroyEvent = null;
        ...
        /// 显示的第一个
        /// </summary>
        public int ItemViewFirstIndex
        {
            get
            {
                return mItemList.Count > 0 ? mItemList[0].ItemIndex : -1;
            }
        }
        ...
        /// 显示的最后一个
        /// </summary>
        public int ItemViewLastIndex
        {
            get
            {
                return mItemList.Count > 0 ? mItemList[mItemList.Count - 1].ItemIndex : -1;
            }
        }
        ...
        public ItemPrefabConfData GetItemPrefabConfData(string prefabName)
        {
            foreach (ItemPrefabConfData data in mItemPrefabDataList)
            {
                ...
                {
                    LogUtlis.Error("A item prefab is null ");
                    continue;
                }
        ...
        */
        public void InitListView(int itemTotalCount,
            ...
            LoopListViewInitParam initParam = null, int initJumpInde = 0)
        {
            if (initParam != null)
            {
            ...
            if (mScrollRect == null)
            {
                LogUtlis.Error("ListView Init Failed! ScrollRect component not found!");
                return;
            }
            if (mDistanceForRecycle0 <= mDistanceForNew0)
            {
                LogUtlis.Error("mDistanceForRecycle0 should be bigger than mDistanceForNew0");
            }
            if (mDistanceForRecycle1 <= mDistanceForNew1)
            {
                LogUtlis.Error("mDistanceForRecycle1 should be bigger than mDistanceForNew1");
            }
            mCurSnapData.Clear();
            mItemPoolDict.Clear();
            ...
            {
                LogUtlis.Error("ScrollRect.horizontalScrollbarVisibility cannot be set to AutoHideAndExpandViewport");
            }
            ...
            {
                LogUtlis.Error("ScrollRect.verticalScrollbarVisibility cannot be set to AutoHideAndExpandViewport");
            }
            ...
            {
                LogUtlis.Warn("LoopListView.InitListView method can be called only once.");
            }
            mListViewInited = true;
            ResetListView();
            //SetListItemCount(itemTotalCount, initJumpInde);
            mCurSnapData.Clear();
        ...
        */
        public void SetListItemCount(int itemCount, int resetPos)
        {
        ...
        public void SetListItemCountNoMove(int itemCount)
        {
            SetListItemCount(itemCount, -1);
        }
        ...
        */
        ...
        public LoopListViewItem NewListViewItem(string itemPrefabName)
        {
            ...
            {
                LogUtlis.Error("没有设置子对象");
                return null;
            }

            itemPrefabName = string.IsNullOrEmpty(itemPrefabName) ? mItemPrefabDataList[0].mItemPrefab.name : itemPrefabName;
            ItemPool pool = null;
        ...
        */
        ...
        void InitItemPool()
        {
            foreach (ItemPrefabConfData data in mItemPrefabDataList)
            {
                ...
                {
                    LogUtlis.Error("A item prefab with name " + prefabName + " has existed!");
                    continue;
                }
                RectTransform rtf = data.mItemPrefab.GetComponent<RectTransform>();
                if (rtf == null)
                {
                    LogUtlis.Error("RectTransform component is not found in the prefab " + prefabName);
                    continue;
                }
                AdjustAnchor(rtf);
                AdjustPivot(rtf);
        ...
        public Vector3 GetItemCornerPosInViewPort(LoopListViewItem item, ItemCornerEnum corner = ItemCornerEnum.LeftBottom)
        {
        ...
        void UpdateCurSnapData()
        {
            ...
            if (mCurSnapData.mSnapStatus == SnapStatus.SnapMoving)
            {
                if (mCurSnapData.mIsForceSnapTo)
                {
                    ...
                    {
                        LoopListViewItem targetItem = GetShownItemNearestItemIndex(mCurSnapData.mSnapTargetIndex);
                        if (targetItem == null)
                        {
                            mCurSnapData.Clear();
                            return;
        ...
        //moveMaxAbsVec param is the max abs snap move speed, if the value <= 0 then LoopListView would use SnapMoveDefaultMaxAbsVec
        ...
        public void UpdateListView(float distanceForRecycle0, float distanceForRecycle1, float distanceForNew0, float distanceForNew1)
        {
            ...
            if (mIsVertList)
            {
                ...
                while (needContinueCheck)
                {
                    ...
                    {
                        LogUtlis.Error("UpdateListView Vertical while loop " + checkCount + " times! something is wrong!");
                        break;
                    }
            ...
            else
            {
                ...
                while (needContinueCheck)
                {
                    ...
                    if (checkCount >= maxCount)
                    {
                        LogUtlis.Error("UpdateListView  Horizontal while loop " + checkCount + " times! something is wrong!");
                        break;
                    }
        ...
        {
            if (onLoopListViewDestroyEvent != null)
            {
                onLoopListViewDestroyEvent(this);
            }
        }
    }
}