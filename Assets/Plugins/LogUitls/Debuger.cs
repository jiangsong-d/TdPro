    ...
    public class Debuger
{
        ...
        private static string NetSendInfoFormat = "<color=#FDB813>[Net] {0}<color=#00BFFF>{1}</color> {2}</color>";
        private static string NetReceiveInfoFormat = "<color=#FFA500>[Net] {0}<color=#00BFFF>{1}</color> {2}</color>";        
        ...
        public static void InfoNetSend(string tag, string message)
        {
            if (LogLevel >= LogLevel.Info)
            {
                string msg = string.Format(NetSendInfoFormat, GetLogTime(), "", message);
                Internal_Log(msg, null);
                WriteToFile(msg, false);
            }
        }
        public static void InfoNetReceive(string tag, string message)
        {
            if (LogLevel >= LogLevel.Info)
            {
                string msg = string.Format(NetReceiveInfoFormat, GetLogTime(), "", message);
                Internal_Log(msg, null);
                WriteToFile(msg, false);
            }
        }
