...
namespace UpdetaFramework
{
    ...
    public class StateMachine
    {
        ...
        /// 状态机持有者
        /// </summary>
        public System.Object Owner { set; get; }
        ...
        public void AddNode(IStateNode stateNode)
        {
            ...
            {
                LogUtlis.Error($"State node already existed : {nodeName}");
            }
        }
        /// <summary>
        /// 转换状态节点
        ...
        public void ChangeState(string nodeName)
        {
            ...
            if (node == null)
            {
                LogUtlis.Error($"Can not found state node : {nodeName}");
                return;
            }

            LogUtlis.Info($"{_curNode.GetType().FullName} --> {node.GetType().FullName}");
            _preNode = _curNode;
            _curNode.OnExit();
            _curNode = node;
            _curNode.OnEnter();
